...
logger = set_logger(os.path.join(LOG_FILEPATH, "log.log"))
...
if __name__ == "__main__":
    ...
    X.assign(rent=y).to_csv(
        ...
        os.path.join(DATA_PATH, "storage", "hous_rent_train_features.csv"),
        index=False,
    )
    ...
    for i, params in enumerate(param_set):
        ...
        with mlflow.start_run(run_name=f"Run {i}"):
            ...
            pipeline = Pipeline(
                ...
                [("proprocessor", preprocess_pipeline), ("regr", regr)]
            )
            pipeline.fit(_X, y)
